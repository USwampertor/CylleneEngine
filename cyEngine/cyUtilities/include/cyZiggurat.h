...
#if CY_COMPILER == CY_COMPILER_GNUC
...
namespace CYLLENE_SDK
{
  ...
  class Ziggurat
  {
    ...
    using ziggurat = ZIGGURAT::normal_ziggurat<T>;
  ...
  public:
    ...
    struct param_type
    {
      using distribution_type = Ziggurat;
    ...
    // stddev.
    explicit Ziggurat(result_type mean, result_type stddev = 1)
      : param_{ mean, stddev }
    {
    }
    ...
    // parameters.
    explicit Ziggurat(param_type const& param)
      : param_{ param }
    {
    }
  ...
  private:
    ...
    {
      constexpr std::size_t bit_count = ZIGGURAT::log2(URNG::max() - URNG::min());

      for (;;)
      {
        auto const bits = ZIGGURAT::generate_bits<bit_count>(random);
        auto const uniform = ZIGGURAT::canonicalize<bit_count, T>(bits);
      ...
      bool check_accept(URNG& random, T lower_edge, T upper_edge, T x) const
    {
      ...
      std::uniform_real_distribution<T> uniform(
        ZIGGURAT::gaussian(lower_edge),
        ZIGGURAT::gaussian(upper_edge)
      );
      return uniform(random) < ZIGGURAT::gaussian(x);
    }
  ...
  {
    typename Ziggurat<T>::param_type param;
    if (is >> param) {
      dist.param(param);
    }
    return is;
  ...
  template<typename T>
  T const ZIGGURAT::normal_ziggurat<T>::edges[] = {
