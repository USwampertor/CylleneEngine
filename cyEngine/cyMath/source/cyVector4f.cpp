...
namespace CYLLENE_SDK {
  ...
  Vector4f::Vector4f(const Vector2i& other, const float& nz, const float& nw)
    : x(static_cast<float>(other.x)),
      y(static_cast<float>(other.y)),
      z(nz),
      w(nw) {}

  Vector4f::Vector4f(const Vector2f& other, const float& nz, const float& nw)
    : x(other.x),
      y(other.y),
      z(nz),
      w(nw) {}

  Vector4f::Vector4f(const Vector3f& other, const float& nw)
    : x(other[0]),
      y(other[1]),
      z(other[2]),
      w(nw) {}
  ...
  Vector4f::distance(const Vector4f& a, const Vector4f& b) {
    return
      Math::sqrt(Vector4f::sqrDistance(a, b)); // Math::sqr(a.x - b.x) +
                                               // Math::sqr(a.y - b.y) +
                                               // Math::sqr(a.z - b.z) +
                                               // Math::sqr(a.w - b.w));
  }
  ...
  Vector4f::magnitude() const {
    return Math::sqrt(this->sqrMagnitude()/*x * x + y * y + z * z + w * w*/);
  }
