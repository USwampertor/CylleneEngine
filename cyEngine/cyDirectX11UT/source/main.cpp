...
main(int argc, char* argv[])
{
  ...
  CCamera* shadowCamera = shadowCameraEntity.createComponent<CCamera>();
  ...
  SPtr<GMesh> cubeMesh;
  {
    ...
    cubeObject.getTransform()->setPosition(Vector3f(0.0f, 0.5f, 0.0f));
    cubeObject.getTransform()->setScale(Vector3f(2.0f, 2.0f, 2.0f));
  }
  ...
  while (running) {
    ...
    auto draw = [&](CCamera* refCamera, BBeing& refObject, const SPtr<GMesh>& refMesh, const SPtr<GTexture>* refTexture) {
    ...
    // Shadow pass
    {
      ...
      GraphicsDX11API::instance().getDeviceContext()->setConstantBuffer(2, 1, gbVector);
    ...
    // Color pass
    {
      ...
      GraphicsDX11API::instance().getDeviceContext()->setConstantBuffer(0, 1, scVector);
      ...
      cubeObject.getTransform()->setPosition(Vector3f(0.0f, 1.5f + Math::cos(time), 0.0f));
    ...
    // Composition
    {
      ...
      GraphicsDX11API::instance().getDeviceContext()->setConstantBuffer(3, 1, gbVector);
