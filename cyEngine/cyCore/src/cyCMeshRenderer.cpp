...
CMeshRenderer::setModel(const SPtr<RModel>& newModel) {
  ...
  for (int i = 0; i < newModel->m_meshes.size(); ++i) {
    ...
    //                                    Vector3f::ONE, 
    //                                    Quaternion::IDENTITY, 
    //                                    std::make_shared<CTransform>(m_owner->getTransform())); // TODO: REVISION ON HOW DO YOU GET TRANSFORM
    child->createComponent<CMeshRenderer>(newModel->m_meshes[i]);
    // m_owner->getTransform()->attachChildren(std::make_shared<CTransform>(child->getTransform()));
  }
}
